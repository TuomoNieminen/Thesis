...
```{r, fig.width = 8, fig.height = 4, fig.cap = "\\label{fig:SPRT-weakness}A graphical illustration of a weakness of Wald's SPRT. The blue line describes the expected observations given the actual relative incidence rate (RR_a) and the green and red lines describe the probability models for the observations under the null and alternative hypotheses, respectively. Left: A choice of point alternative hypothesis (RR_1) close to the rate specified by the null hypothesis (RR_0) and far from RR_a can delay the rejection of H0, because the true expected observations are given low likelihood under both hypotheses. Right: A choice of point alternative hypothesis that is 'too agressive' compared to the actual relative rate can increase the type II error rate, because the null hypothesis gives higher likelihood to the true expected observations."}
...
par(mai=c(0,0,0,0))
plot.new()
legend("center", inset = 0, legend = c("P(Y | RR_0)", "P(Y | RR_1)"), col = cols[1:2], lty = 1, horiz = TRUE, xpd = T)
...
axis(1, act, col = cols[3], label = "E[Y | RR_a]")
abline(v = act, col =  cols[3], lty = 2)

title(sub = "Poor H1 can delay signal generation", main = expression(atop("Both models are 'bad'","P(Y | RR_1)" %~~% "P(Y | RR_0)")))
