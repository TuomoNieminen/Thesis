...
axis(1, act, col = cols[3], label = "RR_a")
...
In the BmaxSPRT experiment, adverse events are classified as a "case" ("success", $Y = 1$) or a "control" ("failure", $Y = 0$), depending on the outcome of a Bernoulli random variable $Y$ (See section \ref{section:simpleSCCS}). Possible transitions in the state space $S$ are therefore as follows:  
...
It should be noted that by this definition $Q$ may contain states which are impossible to reach (because of previous absorbing states). However, this does not cause theoretical or practical problems.
...
where $p$ is as in (\ref{eq:p}). Under the null hypothesis ($RR = 1$) $p = 1 / (1 + z)$. The probabilities of other transitions from the transient states are zero. The transition probabilities for the absorbing states, i.e. for those for which $LLR(n, y_n) \geq c$ or $n = N$, are:
