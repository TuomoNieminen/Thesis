...
The method used in this experiment is the binomial variant of maxSPRT, BmaxSPRT. For details please refer to earlier chapters. In order to deploy the BmaxSPRT, one needs to specify
...
The chosen maximum number of observations affects the power of the experiment. Alternatively to directly specifying 4, the desired power corresponding to the choices made for 2-3 and a specified rate ratio could be chosen instead and the maximum number of events correspoding to that power could be computed. In this experiment the maximum number of observations was directly chosen. Auxiliary information was used to help define the maximum number of observations.
...
The test statistic is the maximized log-likelihood ratio as in (\ref{eq:LLR-BmaxSPRT}). The experiment proceeds as described in algorithm \ref{alg:maxSPRT}. For each day of surveillance, the value of the test statistic is computed and compared to the critical value to make a decision. If at any day during surveillance the value of the test statistic reaches the critical value, the experiment is stopped for the vaccination group and the $H_0$ is rejected. If there are no new observations, the value of the test statistic will remain the same and the decision made during the previous day will not change.  

If the maximum number of observations is reached without rejecting $H_0$, the experiment ends and $H_0$ is accepted.  

### Observation period

The observation period for each individual is an age interval during which the individual can be sampled as a case, starting from some determined date. To define the age intervals, distributions of age at first vaccination were analyzed for each of the vaccination groups. The observation period for each vaccination group was then defined so that the chosen age interval covers most first dose vaccinations (>95%). The observation periods as ages in days for each of the vaccination groups are displayed in table \ref{tab:vaccine-groups}.  
...
Auxiliary information was used to help choose the maximum number of observations $N$ for each vaccination group. The maximum number of observations was chosen according to the expected number of seizures during surveillance, given no change in the incidence during the risk period. The desired type I error rate was chosen to be $\alpha' = 0.05$ and the actual type I errors were approximately the desired rates. The power of the BmaxSPRT experiment was calculated for the choices of $N$ and $\alpha'$ and a choice of rate ratio $RR = 1.5$.  

The parameters defining the stopping conditions of the maxSPRT experiment for each vaccination group are displayed in Table \ref{tab:application-parameters}. It can be seen that power of the experiment is reasonable for each group. However, the actual power of the experiment depends on the actual rate ratio and the actual risk period.  
...
All analyses were carried out using the R program for statistical computing [@R-software]. Functions from the Sequential R package were used for parts of the computations [@R-Sequential], such as computing the critical values and the power of the experiment. In addition to the main results, results for continued surveillance are displayed for MMR and PCV for which a signal was generated and early stopping occured.
...
The rate ratio estimes at the time of signal generation were 1.59 for MMR and 2.15 for PCV. For Rota, the rate ratio estimate at the end of surveillance was 1.01. According to the choices of the risk and control intervals, the expected proportion of cases under the $H_0$ is $1/3$. For MMR there were 66 cases out of 149 events ($\approx 44$%) and for PCV there were 29 cases out of 56 events ($\approx 52$%). For Rota, the proportion of cases ($\approx 34$%) closely matched the expected proportion. These results are gathered in Table \ref{tab:surveillance-results-summary1}.
