...
In a real application of BmaxSPRT surveillance, data is collected and analyzed until a decision to reject or accept the null hypothesis can be made. In this retrospective proof-of-concept application, the goal was to utilize a limited amount of already existing data.  

Finnish children under 2 years of age, born during 2010 - 2014 were considered eligible for this study. For MMR, the 2014 birth cohort was not considered, as due to the higher expected first dose vaccination age, no sufficient data for the 2014 cohort was yet available at the time of the study.
...
The chosen maximum number of observations affects the power of the experiment. Alternatively to directly specifying 4, the desired power corresponding to the choices made for 2-3 and a specified rate ratio could be chosen and the maximum number of events correspoding to that power could then be computed. In this experiment the maximum number of observations was directly chosen. Auxiliary information was used to help define the maximum number of observations.
...
The observation period in BmaxSPRT for each individual is an age interval during which the individual can be sampled as a case, starting from some determined calendar date. To define the age intervals of interest, distributions of age at first vaccination dose were analyzed for each of the vaccination groups. The observation period for each vaccination group was then defined so that the chosen age interval covers most first dose vaccinations (>95%). The observation periods as ages in days for each of the vaccination groups are displayed in Table \ref{tab:vaccine-groups}.  
...
The size of the vaccinated population was estimated using available birth data and vaccination coverage estimates. The expected number of seizures during surveillance, during which each vaccinated individual contributes a total of 42 person days to the experiment, was then computed. These expected numbers of events were then used as the maximum number of observations in the experiment.
...
knitr::kable(df, caption = "\\label{tab:incidence}The included birth cohorts, size of the study population and estimated  vaccination coverage for each of the observed age groups, along with the baseline event incidence estimates given by 1000 person years. The incidence estimates were used to calculate the expected number of events during surveillance, during which vaccinated individuals contribute 42 person days to the experiment.")
```

It should be noted that alternative strategies could have been used. For this retrospective experiment, it would have also been possible to simply count the total number of events during the risk and control periods in the available data for each vaccination group, instead of estimating the expected number of events. One could then choose the smallest of those as the mutual maximum number of observations. It is likely that other plausible solutions exist as well.
...
Inference in the experiment is based on a conditional likelihood as in (\ref{eq:SCCS-binomial}), which can be thought of as independent binomial trials, were the probability of an adverse event being classified as a "case" is a function of the rate ratio parameter $RR$. Higher values of $RR$ correspond to a higher probability that an event occured during the risk period instead of the control period. The test statistic is the maximized log-likelihood ratio as in (\ref{eq:LLR-BmaxSPRT}).  

The experiment proceeds as described in algorithm \ref{alg:maxSPRT}. For each day of surveillance, the value of the test statistic is computed and compared to the critical value to make a decision. If at any day during surveillance the value of the test statistic reaches the critical value, the experiment is stopped for the vaccination group and the $H_0$ is rejected. If there are no new observations, the value of the test statistic will remain the same and the decision made during the previous day will not change. If the maximum number of observations is reached without rejecting $H_0$, the experiment ends and $H_0$ is accepted.  

Auxiliary information was used to help choose the maximum number of observations $N$ for each vaccination group. The maximum number of observations was chosen according to the expected number of seizures during surveillance, computed according to the estimated baseline incidence. The desired type I error rate was chosen to be $\alpha' = 0.05$. The actual type I errors were approximately the desired rates. The power of the BmaxSPRT experiment was calculated for a choice of rate ratio $RR = 1.5$.  
...
All analyses were carried out using the R program for statistical computing [@R-software]. Functions from the Sequential R package were used for parts of the computations [@R-Sequential], such as computing the critical values and the power of the experiment. In addition to the main results of BmaxSPRT surveillance for the three vaccines, results for continued surveillance are illustrated for MMR and PCV.

### Surveillance results

The BmaxSPRT method generated a signal on day 693 of surveillance for PCV and on day 1041 for MMR and the $H_0$ was rejected. No signal was generated for Rota and the maximum number of observations was reached without rejecting $H_0$. Thus the $H_0$ was accepted for Rota. The values of the test statistic and the rate ratio estimates for each day of surveillance are illustrated in Figure \ref{fig:surveillance-results1}).

According to the choices of the risk and control periods, the expected proportion of cases under the $H_0$ is $1/3$. For MMR there were 66 cases out of 149 events ($\approx 44$%) and for PCV there were 29 cases out of 56 events ($\approx 52$%). For Rota, the proportion of cases ($\approx 34$%) closely matched the expected proportion. The rate ratio estimes at the time of signal generation were 1.59 for MMR and 2.15 for PCV. For Rota, the rate ratio estimate at the end of surveillance was 1.01. These results are displayed in Table \ref{tab:surveillance-results-summary1}.  
...
I used simulation to study how the power of BmaxSPRT is affected by the poor choices of risk and control periods. The analysis was carried out by repeadetly simulating adverse events data and applying the BmaxSPRT method to the simulated data with different choices for the risk and control periods. Below I will describe the assumptions of the simulation and then present the results of the analysis. 
...
Table \ref{tab:sim-results} shows the power for each risk and control period pair in the simulation. Power is the proportion of simulation iterations where the null hypothesis was rejected. When the risk and control periods were chosen perfectly as was done with the reference group, the power in the simulation was $0.9$. As expected, the power is lower the more the choices for the risk and control periods differ from the reference period. For example, if the chosen risk period is approximately 2.5 times the size of the actual risk period (0-30 compared to 0-13), the power decreases from 0.9 to 0.35.  
...
knitr::kable(cbind(sim$choices[, c(1,3)], df), caption = "\\label{tab:sim-results}Results of the simulation. The power for the reference group matches the desired power of the experiment. The power is lower the more the choices for the risk and control periods differ from the reference period.")
```
